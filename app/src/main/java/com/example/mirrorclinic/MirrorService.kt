 ...
 *
 * A floating "Mirror" button sits in the top-right corner over every app.
 * Tapping it captures ONE frame of the current screen via MediaProjection,
 * then shows that frozen frame flipped horizontally in a full-screen overlay.
 *
 * Because we capture a single frame and then stop capturing, there is no
 * feedback loop -- that is the trick that makes this possible without root.
 */
class MirrorService : Service() {
    ...
    private fun showBubble() {
        ...
        val btn = Button(this).apply {
            text = "Mirror"
            setOnClickListener { captureAndShow() }
        }
